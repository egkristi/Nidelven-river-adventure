...
def render_3d_perspective(
    ...
    ax.plot_surface(
        X[::step, ::step], Y[::step, ::step], dem_data[::step, ::step],
    ...
    if river_path is not None and river_path.shape[1] >= 2:
        ...
        ax.plot(
            river_path[:, 1], river_path[:, 0], path_elevations,
            'b-', linewidth=3, alpha=0.9, label='River'
        )
        
        # Start/end markers
        ax.plot(
            [river_path[0, 1]], [river_path[0, 0]], [path_elevations[0]],
            'g^', markersize=10, label='Start'
        )
        ax.plot(
            [river_path[-1, 1]], [river_path[-1, 0]], [path_elevations[-1]],
            'rv', markersize=10, label='End'
        )
...
def generate_preview_images(
    ...
    # River profile
    if flow_data:
        render_river_profile(
            flow_data,
            output_path=output_dir / "river_profile.png"
        )
