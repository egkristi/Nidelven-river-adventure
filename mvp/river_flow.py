...
def trace_river_path(
    ...
    for _ in range(max_steps):
        ...
        # Check boundaries
        if (row < boundary_margin or row >= h - boundary_margin or
            col < boundary_margin or col >= w - boundary_margin):
            break
...
def create_river_from_dem(
    ...
    """
    from terrain_mesh import load_dem, generate_mesh
    ...
    if len(path) < 2:
        ...
        h, w = dem_data.shape
        path = [(int(h * i / num_points), int(w * i / num_points)) 
    ...
    print(f"  Velocity range: {flow_data['velocities'].min():.1f} - {flow_data['velocities'].max():.1f} m/s")
...
if __name__ == "__main__":
    ...
    print(f"  Total elevation drop: {flow_data['elevations'].max() - flow_data['elevations'].min():.1f}m")
