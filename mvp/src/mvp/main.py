...
def main():
    ...
    parser.add_argument('--download', action='store_true', 
                        help='Try to download real DEM from Kartverket')
    parser.add_argument('--sample', action='store_true',
                        help='Force use of synthetic DEM data')
    parser.add_argument('--interactive', action='store_true',
                        help='Launch interactive 3D renderer (requires display)')
    parser.add_argument('--output-dir', type=Path, default=None,
                        help='Output directory for generated files')
    parser.add_argument('--size', type=int, default=512,
                        help='DEM size in pixels (default: 512)')
    parser.add_argument('--skip-terrain', action='store_true',
                        help='Skip terrain mesh generation')
    parser.add_argument('--skip-river', action='store_true',
                        help='Skip river flow calculation')
        ...
        try:
            ...
            if terrain_mesh and flow_data:
                ...
                river_mesh = generate_river_mesh(
                    flow_data, dem_data, terrain_mesh
                )
                
                run_renderer(
                    terrain_mesh=terrain_mesh,
                    water_mesh=river_mesh,
                    river_path=river_path
                )
            else:
