...
class NidelvenRenderer(mglw.WindowConfig):
    ...
    def load_mesh(self, mesh_data: dict, shader_type: str = "terrain"):
        ...
        if shader_type == "terrain":
            program = self.ctx.program(
                vertex_shader=TERRAIN_VERTEX_SHADER, fragment_shader=TERRAIN_FRAGMENT_SHADER
            )
        else:
            program = self.ctx.program(
                vertex_shader=WATER_VERTEX_SHADER, fragment_shader=WATER_FRAGMENT_SHADER
            )

        vao = self.ctx.vertex_array(
            program, [(vbo, "3f 3f 2f", "in_position", "in_normal", "in_uv")], ibo
        )
    ...
    def get_look_at(self) -> np.ndarray:
        ...
        look_ahead_idx = min(
            int(self.river_progress * (len(self.river_path) - 1)) + 10, len(self.river_path) - 1
        )
