...
class RiverCamera:
    ...
    def __init__(self):
        ...
        # River following
        self.river_path: np.ndarray | None = None
    ...
    def set_river_path(self, path: np.ndarray, elevations: np.ndarray | None = None):
        """
    ...
    def update(self, dt: float):
        ...
        self._smooth_target = self._lerp(self._smooth_target, self.target, self.target_smooth)
    ...
    def get_projection_matrix(
        self, aspect_ratio: float, fov: float = 60.0, near: float = 0.1, far: float = 10000.0
    ) -> np.ndarray:
...
class OrbitCamera:
    ...
    def get_projection_matrix(
        self, fov: float = 60.0, near: float = 0.1, far: float = 10000.0
    ) -> np.ndarray:
    ...
    def zoom(self, delta: float):
        ...
        self.distance = np.clip(
            self.distance + delta * self.zoom_speed, self.min_distance, self.max_distance
        )

    def pan(self, delta_x: float, delta_y: float):
        """Pan target position."""
        right = np.array([np.cos(self.azimuth), 0.0, -np.sin(self.azimuth)], dtype=np.float32)
